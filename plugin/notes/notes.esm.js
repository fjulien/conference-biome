...
class _ {
	...
	autolink(t, e) {
		...
		if (n) {
			...
			return (
				"@" === n[2]
					? ((t = l(this.options.mangle ? e(n[1]) : n[1])), (r = "mailto:" + t))
					: ((t = l(n[1])), (r = t)),
				{
					type: "link",
					raw: n[0],
			...
			if (
				...
				switch (d.type) {
					...
					case "table":
						...
						for (
							c += this.renderer.tablerow(l), u = "", s = d.rows.length, r = 0;
							r < s;
							r++
						) {
	...
	return (n, r, i) => {
		...
		if (r.async)
			return Promise.resolve(r.hooks ? r.hooks.preprocess(n) : n)
				...
				.then((t) =>
					r.walkTokens
						? Promise.all(O.walkTokens(t, r.walkTokens)).then(() => t)
						: t,
				)
				.then((t) => e(t, r))
...
(O.options = O.setOptions =
	...
	(O.walkTokens = function (t, e) {
		let n = [];
		for (const r of t)
			switch (((n = n.concat(e.call(O, r))), r.type)) {
				case "table":
					for (const t of r.header) n = n.concat(O.walkTokens(t.tokens, e));
					for (const t of r.rows)
						for (const r of t) n = n.concat(O.walkTokens(r.tokens, e));
					break;
				case "list":
					n = n.concat(O.walkTokens(r.items, e));
					break;
				default:
					O.defaults.extensions &&
					O.defaults.extensions.childTokens &&
					O.defaults.extensions.childTokens[r.type]
						? O.defaults.extensions.childTokens[r.type].forEach(function (t) {
								n = n.concat(O.walkTokens(r[t], e));
						  })
						: r.tokens && (n = n.concat(O.walkTokens(r.tokens, e)));
			}
		return n;
	}),
						...
						: window.addEventListener("message", (t) => {
								if (!n && "string" == typeof t.data) {
									...
									r &&
										...
										n && !n.closed
											? n.focus()
											: ((n = e), window.addEventListener("message", s), a()));
								}
								var e;
						  }),
					e.addKeyBinding(

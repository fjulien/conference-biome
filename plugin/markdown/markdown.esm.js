...
const n = /[&<>"']/;
const s = new RegExp(n.source, "g");
const r = /[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/;
const i = new RegExp(r.source, "g");
const l = {
	"&": "&amp;",
	"<": "&lt;",
	">": "&gt;",
...
function z(e, t, n, s) {
	const r = t.href;
	const i = t.title ? a(t.title) : null;
		...
		if (t) {
			...
			while (e && ((g = !1), (t = x.exec(e))) && !this.rules.block.hr.test(e)) {
				if (
					...
					const r = new RegExp(`^ {0,${Math.min(3, i - 1)}}(?:\`\`\`|~~~)`);
					const l = new RegExp(`^ {0,${Math.min(3, i - 1)}}#`);
					while (
						e &&
						((p = e.split("\n", 1)[0]),
						(h = p),
...
class v {
	...
	inlineTokens(e, t = []) {
			...
			if (e.length > 0)
				while (null != (i = this.tokenizer.rules.inline.reflinkSearch.exec(a)))
					e.includes(i[0].slice(i[0].lastIndexOf("[") + 1, -1)) &&
						(a = `${a.slice(0, i.index)}[${$("a", i[0].length - 2)}]${a.slice(
		...
		while (null != (i = this.tokenizer.rules.inline.escapedEmSt.exec(a)))
			(a = `${a.slice(0, i.index + i[0].length - 2)}++${a.slice(
	...
	return (n, s, r) => {
		...
		const l = ((e, t, n) => (s) => {
			if (
				...
				const e = `<p>An error occurred:</p><pre>${a(
					`${s.message}`,
					!0,
				)}</pre>`;
		...
		if (
			...
			if (((s.highlight = undefined), !o.length)) return a();
			let c = 0;
			return (
				M.walkTokens(o, (e) => {
					"code" === e.type &&
...
const B = () => {
	...
	function r(e, t) {
		...
		return (
			...
			`<script type="text/template">${(e = e.replace(
				/<\/script>/g,
				N,
			))}<\/script>`
		);
	...
	function i(e, t) {
		...
		const i = new RegExp(t.separator);
		let l;
		let o;
		let a;
		let c = 0;
		...
		for (let e = 0, n = p.length; e < n; e++)
			Array.isArray(p[e])
				? ((u += `<section ${t.attributes}>`),
					...
					(u += "</section>"))
				: (u += `<section ${t.attributes} data-markdown>${r(
						p[e],
						t,
					)}</section>`);
		return u;
						...
						? r.push(
								((e) =>
									new Promise(function (t, n) {
										...
										const r = e.getAttribute("data-markdown");
										const i = e.getAttribute("data-charset");
										null !== i &&
											"" !== i &&
										...
										try {
											...
											console.warn(
												`Failed to get the Markdown file ${r}. Make sure that the presentation and the file are served by a HTTP server and the file can be found there. ${e}`,
											),
												t(s, r);
										}
									}))(e).then(
	...
	function o(e, t, n) {
		...
		if ((i = s.exec(o))) {
			const n = i[1];
			for (
				o = o.substring(0, i.index) + o.substring(s.lastIndex), e.nodeValue = o;
				(l = r.exec(n));
			)
